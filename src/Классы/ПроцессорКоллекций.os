...
Перем РегулярноеВыражениеРезультата;
...
Процедура ДляКаждого(Знач ФункцияОбработки, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ФункцияОбработки) = Тип("Строка") Тогда
		ФункцияОбработки = СформироватьВременноеОписаниеОповещения(ФункцияОбработки, ДополнительныеПараметры, Ложь);
	КонецЕсли;
	ДополнитьСтруктуру(ФункцияОбработки.ДополнительныеПараметры, ДополнительныеПараметры, Истина);
...
Функция СформироватьВременноеОписаниеОповещения(Знач ПользовательскоеВыражение, Знач ДополнительныеПараметры,
												Знач ПодставлятьСтрокуУстановкиРезультата = Истина)

	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = Новый Структура;
	ИначеЕсли ДополнительныеПараметры.Свойство("ДополнительныеПараметры") Тогда
		ВызватьИсключение "Структура ДополнительныеПараметры не должна включать ключ с именем ДополнительныеПараметры";
	КонецЕсли;

	Если ПодставлятьСтрокуУстановкиРезультата И Не РегулярноеВыражениеРезультата.Совпадает(ПользовательскоеВыражение) Тогда
		ПользовательскоеВыражение = СтрШаблон("Результат = %1;", ПользовательскоеВыражение);
	КонецЕсли;

	ТекстИзолированногоКласса = ТекстШаблонаИзолированногоКласса;
	...
	ТекстИзолированногоКласса = СтрЗаменить(
		ТекстИзолированногоКласса,
		"РазвернутьДополнительныеПараметры();",
		ТекстВстраиванияДополнительныхПараметров(ДополнительныеПараметры)
	);
	Лог.Отладка(ТекстИзолированногоКласса);
...
Функция ТекстВстраиванияДополнительныхПараметров(Знач ДополнительныеПараметры)
	Строки = Новый Массив;
	Для каждого КлючЗначение Из ДополнительныеПараметры Цикл
		Строки.Добавить(СтрШаблон("%1 = ДополнительныеПараметры.%1;", КлючЗначение.Ключ));
	КонецЦикла;
...
Процедура Инициализация()
	...
	РегулярноеВыражениеРезультата = Новый РегулярноеВыражение("\s*Результат\s*=");
	РегулярноеВыражениеРезультата.ИгнорироватьРегистр = Истина;
